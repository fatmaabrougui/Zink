...
function afficherevent ($event){
	...
	function modifierevent($event,$nom_event){
		$sql="UPDATE event SET nom_event=:nom_event,type_event=:type_event,date_event=:date_event WHERE nom_event=:nom";
...
try{		
        ...
        $nom_event1=$event->getnom_event();
        $type_event=$event->gettype_event();
		$date_event=$event->getdate_event();
		
		$datas = array(':nom_event'=>$nom_event1,':type_event'=>$type_event,':date_event'=>$date_event,':nom'=>$nom_event);
		$req->bindValue(':nom',$nom_event);
		$req->bindValue(':nom_event',$nom_event1);
		$req->bindValue(':type_event',$type_event); 
		$req->bindValue(':date_event',$date_event);
